...
void TAARenderer::initResource()
{
	...
	addTexture("DepthMap", 1024, 1024, GL_DEPTH_COMPONENT);// 创建帧缓冲附件
	GLuint depthMapFBO;
	glGenFramebuffers(1, &depthMapFBO); // 创建帧缓冲
	glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D,
		coreTexture2dArr["DepthMap"]->getTextureID(), 0); // 绑定帧缓冲附件 
	...
	addTexture("VelocityFrameBuffer", screenWidth, screenHeight, GL_RGB, GL_FLOAT);
	...
	addShaderProgram("PostProcessShader", "./src/GLSL/TAA/postprocess.vert", "./src/GLSL/TAA/postprocess.frag");
...
void TAARenderer::initShaderContent()
{
	...
	mat3 ycocgToRgb(0.25, 0.5, 0.25, 0.5, 0, -0.5, -0.25, 0.5, -0.25);
	coreShaderArr["TaaRenderShader"]->setMat3("RGBToYCoCgMat", transpose(ycocgToRgb));
	mat3 rgbToYcocg(1.0, 1.0, -1.0, 1.0, 0.0, 1.0, 1.0, -1.0, -1.0);
	coreShaderArr["TaaRenderShader"]->setMat3("YCoCgToRGBMat", transpose(rgbToYcocg));
...
void TAARenderer::renderScene(Shader& shader)
{
	...
	// cubes
	model = glm::mat4(1.0f);
	model = glm::translate(model, vec3(2.0f, 1.5f, 2.2f));
	model = scale(model, vec3(0.5f));
	model = rotate(model, PI/2, vec3(1, 0, 0));
	shader.setMat4("model", model);
	currentModelMatrixMap[modelMatrixIndex++] = model;
	if (!lastModelMatrixQueue.empty())
	{
...
void TAARenderer::Render_Lighting()
{
	...
	JitteringProjection(*coreShaderArr["LightingShader"], true);
...
void TAARenderer::Render_Velocity()
{
	...
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
	frameUpdateViewProjectionMatrix();
	JitteringProjection(*coreShaderArr["VelocityShader"], false);
...
void TAARenderer::Render_WithoutJitter()
{
	...
	glm::mat4 model;
	model = glm::mat4(1.0f);
	model = glm::translate(model, vec3(2.0f, 1.5f, 1.0f));
	model = scale(model, vec3(0.5f));
	model = rotate(model, PI / 2, vec3(1, 0, 0));
...
void TAARenderer::JitteringProjection(Shader& shader, bool forward)
{
	if (forward)

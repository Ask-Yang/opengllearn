...
2. 透视变换它是非线性的，因为除了z。直观来说，就是空间被挤压了。另外它有两个好的特性，线还是线，面还是面。这推出以下两个推论：
   * 线框确实是被挤压的，本来平行变相交，我们要的就是这种效果，所以是对的。
   * 但是纹理作为一个平面，虽然还是平面，但是会被挤压在一起。想要得到正确的纹理插值，就需要算出点对应的摄像机坐标，反正就是变换到线性空间进行插值，这样得到的结果就是正确的。具体的实现，可以通过屏幕坐标推导，也可以在光栅化时直接保留z值，然后将其变回去。

任何变换如果把它作为一个空间（可以简单的看成一个体积，正方体啥的）去理解会直观很多，线性变换就是一个空间旋转缩放，齐次变换就是将这个空间移动，透视变换就是将一个平头截体压缩成了一个矩形体。
